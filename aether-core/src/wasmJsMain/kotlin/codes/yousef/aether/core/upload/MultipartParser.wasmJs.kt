...
/**
 * WasmJS implementation of multipart parser.
 */
actual class MultipartParser actual constructor(
    ...
    private fun parseMultipartBody(body: ByteArray, boundary: String): List<MultipartPart> {
        ...
        val boundarySize = boundaryBytes.size
        ...
        while (pos < bodySize) {
            if (pos < bodySize - 1 && body[pos] == '\r'.code.toByte() && body[pos + 1] == '\n'.code.toByte()) {
                pos += 2
            } else if (pos < bodySize && (body[pos] == '\r'.code.toByte() || body[pos] == '\n'.code.toByte())) {
                pos++
            }

            if (pos >= bodySize) break
            ...
            pos = nextBoundary + boundarySize
            
            if (pos < bodySize - 1 && body[pos] == '-'.code.toByte() && body[pos + 1] == '-'.code.toByte()) {
                break
            }
        }
